...
app.controller('drillDownController',function($scope,$http,moment,$rootScope,$stateParams,ModalService){
    ...
    $scope.onPageChanged = function(result){
    ...
    $scope.getOverviewDetails = function(){
        ...
        $scope.getRequestUrl = '/statement/details';
        $scope.resultKey = 'STATEMENTS';
        $scope.states && $scope.states.length >0 ? $scope.queryParams.states = _.join($scope.states,',') :'';
        $scope.brands && $scope.brands.length >0 ? $scope.queryParams.brand = _.join($scope.brands,',') :'';
        $scope.invoiceNumber ? $scope.queryParams.invoiceNumber = $scope.invoiceNumber :'';
        $http.get($scope.getRequestUrl,{params:$scope.queryParams}).then(function success(result){
            $scope.searchResults = result.data[$scope.resultKey]
            $scope.totalResults = Math.ceil(result.data.TOTAL/10);
